...
with st.expander("ℹ️ Анықтама"):
    ...
    - **2-кезең**: Қысым, энергия және шығын болжамдарын көріңіз.
    - **3-кезең**: Тұщыландыру процесін реттеңіз және шығындарды есептеңіз.
    - **4-кезең**: Өңірлер бойынша шығындарды салыстырыңыз.
    """)
...
with tab6:
    ...
    **Формулалар:**
    ...
    if example is None:
        ...
        # Optimization
        st.subheader("🔧 Оңтайландыру")
        st.markdown("Процесті оңтайландырыңыз, мақсатты тұздылық ауқымын таңдап алыңыз.")

        # Select optimization objective
        optimization_goal = st.selectbox(
            "Оңтайландыру мақсаты:",
            ["Шығындарды азайту", "Энергия тұтынуды азайту", "Шығындар мен энергияның балансын сақтау"],
        ...
        with col_sal1:
            min_salinity = st.number_input("Минималды мақсатты тұздылық (ppm)", 100, 500, 300, step=10)
        with col_sal2:
            max_salinity = st.number_input("Максималды мақсатты тұздылық (ppm)", min_salinity, 500, 500, step=10)
        ...
        def objective_function(params, goal=optimization_goal):
            ...
            if goal == "Шығындарды минимизациялау":
                return cost + penalty
            elif goal == "Энергия тұтынуды минимизациялау":
        ...
        if result.success:
            ...
            comparison_df = pd.DataFrame({
                ...
                "Ағымдағы": [r_nano, r_ro, input_pressure, sal_ro, total_energy, operational_cost],
                "Оңтайлы": [opt_r_nano, opt_r_ro, opt_pressure, opt_sal_ro, opt_energy, opt_cost]
            })
            comparison_df["Ағымдағы"] = comparison_df["Ағымдағы"].round(2)
            comparison_df["Оңтайлы"] = comparison_df["Оңтайлы"].round(2)
            st.subheader("Ағымдағы және оңтайландырылған параметрлерді салыстыру")
            st.dataframe(comparison_df, use_container_width=True)

            # Visualization
            st.subheader("Оптимизацияның визуализациясы")
            fig_opt = go.Figure(data=[
                go.Bar(name="Ағымдағы", x=["R_nano", "R_ro", "Қысым", "Тұздылық", "Энергия", "Шығын"],
                       y=[r_nano, r_ro, input_pressure, sal_ro, total_energy, operational_cost]),
                go.Bar(name="Оңтайлы", x=["R_nano", "R_ro", "Қысым", "Тұздылық", "Энергия", "Шығын"],
                       y=[opt_r_nano, opt_r_ro, opt_pressure, opt_sal_ro, opt_energy, opt_cost])
            ])
            fig_opt.update_layout(
                title="Ағымдағы және оптималды параметрлерді салыстыру",
                barmode='group',
                template=theme,
                yaxis_title="Мәні",
                height=500
            ...
            st.error("Оңтайландыру сәтсіз аяқталды. Мүмкін себептері:")
            st.markdown("""
            - Мақсатты тұздылық қазіргі параметрлермен қол жеткізілмейді. Төмендегілерді байқап көріңіз:
              - Мақсатты тұздылық диапазонын арттырыңыз.
              - Фильтрация тиімділігін (R_nano, R_ro) слайдерлерде арттырыңыз.
            - Параметрлерге қойылатын шектеулер тым қатал. Кіріс мәндерін тексеріңіз.
            """)
